...
// confirmUser confirms a new user
var confirmUser app.Handler = func(w http.ResponseWriter, r *http.Request) *app.Error {
	...
	uUUID, err := uuid.FromString(claims.Subject)
	if err != nil {
	...
	if err := db.SetUserConfirmed(r, uUUID); err != nil {
		return &app.Error{Code: http.StatusInternalServerError, Message: "internal error"} // already logged
	}
	log.Sugar.Infof("User %s successfully confirmed!", uUUID.String())
